...
def test_checks():
    ...
    with pytest.raises(TypeError):
        Transformer.check_dataframe(dd.from_pandas(pd.DataFrame({"A": [1], 0: ["x"]}), npartitions=1))
    with pytest.raises(TypeError):
        Transformer.check_y(dd.from_pandas(pd.DataFrame({"A": [1], 0: ["x"]}), npartitions=1), [])
    with pytest.raises(TypeError):
        Transformer.check_y(
    ...
    with pytest.raises(ValueError):
        Transformer.check_dataframe_is_numerics(dd.from_pandas(pd.DataFrame({"A": [1], "x": ["x"]})), npartitions=1)
    with pytest.raises(ValueError):
        Transformer.check_binary_target(X, dd.from_pandas(pd.Series([1, 2, 3], name="TARGET"), npartitions=1))
    with pytest.raises(ValueError):
        Transformer.check_multiclass_target(dd.from_pandas(pd.Series([1.1, 2.2, 3.3], name="TARGET"), npartitions=1))
    with pytest.raises(ValueError):
        Transformer.check_regression_target(dd.from_pandas(pd.Series([1, 0, 0], name="TARGET"), npartitions=1))
    with pytest.raises(ValueError):
        Class().check_nans(dd.from_pandas(pd.DataFrame({"A": [np.nan]}), npartitions=1), columns=["A"])
    with pytest.raises(ValueError):
        Class().check_dataframe_with_objects(dd.from_pandas(pd.DataFrame({"A": [1.1], "B": [0]}), npartitions=1))
    with pytest.raises(ValueError):
