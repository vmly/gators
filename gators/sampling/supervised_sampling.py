...
class SupervisedSampling(TransformerXY):
    ...
    * koalas transform
    ...
    def transform(self, X: DataFrame, y: Series) -> Tuple[DataFrame, Series]:
        ...
        def f(x, frac_dict):
            return x.sample(frac=frac_dict[x[y.name].iloc[0]], random_state=self.random_state)

        Xy = X.join(y)
        Xy_sampled = Xy.groupby(y.name).apply(f, self.frac_dict).reset_index(drop=True)
