...
def data():
    ...
    y = dd.from_pandas(pd.Series([0, 1, 1, 1, 0, 0, 0, 0, 1, 1], name="TARGET"), npartitions=1)
    X_expected = X[["A", "B"]].compute().copy()
    model = XGBClassifier(
        random_state=0,
        subsample=1.0,
        n_estimators=2,
...
def data_ks():
    ...
    y = ks.Series([0, 1, 1, 1, 0, 0, 0, 0, 1, 1], name="TARGET"), npartitions=1)
