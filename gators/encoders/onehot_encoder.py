...
class OneHotEncoder(_BaseEncoder):
    ...
    * fit with `koalas` & transform with `NumPy`
    ...
    def transform(self, X: DataFrame) -> DataFrame:
        ...
        X_new = util.get_function(X).get_dummies(X_new, self.columns, prefix_sep="__")
        columns_to_add = [c for c in X.columns if c not in self.all_columns]
        # X_new = X_new.reindex(columns=self.all_columns, fill_value=0.0)
        # reindex is not defined in dask, hack:
        X_new = X_new.join(
            util.get_function(X).fill_dataframe(X, columns=columns_to_add, val=0)
        )[self.all_columns]
